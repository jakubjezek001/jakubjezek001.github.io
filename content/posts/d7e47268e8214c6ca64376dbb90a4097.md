---
title: You are coding assistant who is specialized in writing pytest.
slug: d7e47268e8214c6ca64376dbb90a4097
tags:
  - python
  - dev
date: 2024-12-13T07:47:30.288Z
---

# You are coding assistant who is specialized in writing pytest.

You are coding assistant who is specialized in writing pytest.

```python YNPUT/ayon-traypublisher/client/ayon_traypublisher/plugins/create/create_editorial_advanced.py
import os
import re
import clique
...
following code needs to get some coverage. Focus mainly on `EditorialAdvancedCreator` class and its `create` method. Also you will need to mock some of the `otio` library functions. You can use `unittest.mock` library for that. Also you will need to mock file structure corresponding to the `otio` file content. please do not mock internal functions of the `EditorialAdvancedCreator` class.

I'll help you create a test for the `EditorialAdvancedCreator` class focusing on the `create` method. Here's a proposed test structure:
...
from ayon_traypublisher.plugins.create.create_editorial_advanced import (
...
def mock_folder_structure(tmp_path):
    """Create mock folder structure for testing."""
    # Create main folders
    media_folder = tmp_path / "media"
    media_folder.mkdir()
    
    # Create clip folders
    shot_folder = media_folder / "shot_010"
    shot_folder.mkdir()
    
    # Create plate folder with content
    plate_folder = shot_folder / "plate"
    plate_folder.mkdir()
    
    # Create test files
    (plate_folder / "plate_v001.mov").touch()
    
    return str(media_folder)

@pytest.fixture
def mock_otio_timeline():
    """Create mock OTIO timeline."""
    timeline = mock.MagicMock(spec=otio.schema.Timeline)
    track = mock.MagicMock(spec=otio.schema.Track)
    clip = mock.MagicMock(spec=otio.schema.Clip)
    
    # Setup clip
    clip.name = "shot_010"
    clip.range_in_parent.return_value = mock.MagicMock(
        start_time=mock.MagicMock(value=1001),
        end_time_inclusive=mock.MagicMock(return_value=mock.MagicMock(value=1024))
    )
    clip.duration.return_value = mock.MagicMock(value=24)
    clip.trimmed_range.return_value = mock.MagicMock(
        start_time=mock.MagicMock(value=1001)
    )
    
    # Setup track
    track.name = "Test Track"
    track.find_clips.return_value = [clip]
    
    # Setup timeline
    timeline.video_tracks.return_value = [track]
    timeline.find_clips.return_value = [clip]
    timeline.name = "Test Timeline"
    
    return timeline

@pytest.fixture
def creator(mock_folder_structure):
    """Create EditorialAdvancedCreator instance."""
    creator = EditorialAdvancedCreator()
    creator.log = mock.MagicMock()
    creator.project_name = "test_project"
    creator.create_context = mock.MagicMock()
    creator.create_context.creators = {
        "editorial_shot": mock.MagicMock(spec=EditorialShotInstanceCreator)
    }
    creator.apply_settings(MOCK_PROJECT_SETTINGS)
    return creator

@mock.patch("ayon_api.get_folder_by_path")
@mock.patch("opentimelineio.adapters.read_from_file")
def test_create_editorial_advanced(
    mock_read_from_file,
    mock_get_folder,
    creator,
    mock_otio_timeline,
    ...
    """Test create method of EditorialAdvancedCreator."""
    # Setup mocks
    mock_read_from_file.return_value = mock_otio_timeline
    ...
    pre_create_data = {
        "sequence_filepath_data": [{
            "directory": str(Path(mock_folder_structure).parent),
            "filenames": ["test.edl"]
        }],
        "folder_path_data": [{
            "directory": mock_folder_structure,
            "filenames": ["media"]
        }],
        "fps": "from_selection",
        "plateMain": True,
        "ignore_clip_no_content": True,
        "timeline_offset": 0,
    ...
    mock_get_folder.assert_called_once_with("test_project", "/test/folder")
    
    # Verify instance creation
    creator.create_context.creators["editorial_shot"].create.assert_called_once()
...
1. Creates necessary fixtures:
   - `mock_folder_structure`: Creates a temporary folder structure mimicking the expected file organization
   - `mock_otio_timeline`: Creates a mock OTIO timeline with necessary attributes and methods
   - `creator`: Sets up the EditorialAdvancedCreator instance with required mocks

2. The main test function:
   - Mocks external dependencies (ayon\_api, otio)
   - Sets up test data
   - Calls the create method
   - Verifies expected interactions

You might want to add more test cases to cover different scenarios:

1. Different FPS settings
2. Different folder structures
3. Various clip configurations
4. Error cases
5. Different product type presets

Would you like me to add any specific test cases or explain any part in more detail?
